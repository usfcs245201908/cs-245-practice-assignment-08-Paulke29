...
class BST <T>{
    ...
    // initial TreeNode in the constructor
    public BST() {
        root = null;
    }
   // insert node into tree
    public void insert(Comparable k){
        //checking whether node is null or null;
        if (root == null) {
    ...
    //Finding the node from BST
    public boolean find(Comparable k) {
        return find(root, k);
    }
    //print out the whole BST
    public void print() {
        print(root);
    }
    ...
    // finding the smallest Node in the BinaryTreeNode
    ...
    // following the condition to delete the Node
...
//            T.setRight( delete(T.getRight(), tmp.getValue()) );
            return T;
